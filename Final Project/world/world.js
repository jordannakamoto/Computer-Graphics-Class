...
// Spaghetti Code that contains all front-end logic. I should organize it at some point
// but I have to redo everything anyway because server side inference is too slow
// ---------------------------------------------------------------- //
...
const toggleGuidelinesButton = createIconButton('fas fa-circle-info', 'Toggle Guidelines', toggleGuidelines);
...
let yoloProcessingActive = false;

function toggleGuidelines() {
    const guidelinesOverlay = document.getElementsByClassName('guideline-score');
    // Toggle visibility
    showGuidelines = !showGuidelines;
    for (let i = 0; i < guidelinesOverlay.length; i++) {
        if (showGuidelines) {
            guidelinesOverlay[i].style.visibility = 'visible';
            guidelinesOverlay[i].style.opacity = '1';
        } else {
            guidelinesOverlay[i].style.visibility = 'hidden';
            guidelinesOverlay[i].style.opacity = '0';
        }
    }
}
...
// Person for YOLO Base Model Testing
...
function updateGuidelines(drivabilityScores, obstacles, goalPosition) {
    ...
    if (goalScreenPos) {
        ...
        for (let i = 0; i < numSlices; i++) {
            ...
            const weight = 0.05; // Adjust weight to emphasize goal proximity
    ...
    for (let i = 0; i < numSlices; i++) {
        ...
        if (!showGuidelines) continue; // Skip updates if guidelines are hidden
...
async function processCanvas() {
    ...
    try {
        ...
        // if (showGuidelines) {
        cvWorker.postMessage({ pixels: grayscalePixels, width, height });
        // }
